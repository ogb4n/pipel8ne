...
const TriggerForm: React.FC<{ p: PP; set: (k: string, v: unknown) => void }> = ({ p, set }) => (
  <FieldGroup>
    <div>
      ...
      <Select
        value={(p.triggerType as string) ?? "push"}
    ...
    <FieldGroup>
      <div>
        ...
        <Select value={action} onChange={(e) => set("action", e.target.value)}>
          {(["build", "run", "push", "pull", "compose_up", "compose_down"] as DockerAction[]).map(
            (a) => (
      ...
      {action === "build" && (
        <>
          <div>
            ...
            <Input
              value={(p.dockerfile as string) ?? "Dockerfile"}
          ...
          <div>
            ...
            <Input
              value={(p.buildContext as string) ?? "."}
      ...
      {(action === "compose_up" || action === "compose_down") && (
        <div>
          ...
          <Input
            value={(p.composeFile as string) ?? "docker-compose.yml"}
            onChange={(e) => set("composeFile", e.target.value)}
          />
        </div>
      )}
    </FieldGroup>
    ...
    <FieldGroup>
      ...
      {action === "clone" && (
        <>
          <div>
            ...
            <Input
              value={(p.repositoryUrl as string) ?? ""}
      ...
      {(action === "push" || action === "pull" || action === "fetch") && (
        <div>
          <Label>Remote</Label>
          <Input
            value={(p.remote as string) ?? "origin"}
...
const TestForm: React.FC<{ p: PP; set: (k: string, v: unknown) => void }> = ({ p, set }) => (
  <FieldGroup>
    <div>
      <Label>Runner</Label>
      <Select
        value={(p.runner as TestRunner) ?? "jest"}
    ...
    <div>
      ...
      <Input
        value={(p.testPattern as string) ?? ""}
        onChange={(e) => set("testPattern", e.target.value)}
        placeholder="**/*.test.ts"
      />
    </div>
    <div>
      ...
      <Input
        type="number"
        value={(p.coverageThreshold as number) ?? ""}
        onChange={(e) =>
          set("coverageThreshold", e.target.value ? Number(e.target.value) : undefined)
        }
        placeholder="80"
        min={0}
        max={100}
...
const BuildForm: React.FC<{ p: PP; set: (k: string, v: unknown) => void }> = ({ p, set }) => (
  <FieldGroup>
    ...
    <div>
      ...
      <Input
        value={(p.runtimeVersion as string) ?? ""}
      ...
      {target === "kubernetes" && (
        <>
          <div>
            ...
            <Input
              value={(p.namespace as string) ?? "default"}
          ...
          <div>
            ...
            <Input
              value={(p.manifestPath as string) ?? ""}
          ...
          <div>
            ...
            <Select
              value={(p.rolloutStrategy as RolloutStrategy) ?? "rolling"}
              onChange={(e) => set("rolloutStrategy", e.target.value)}
            >
...
const NotificationForm: React.FC<{ p: PP; set: (k: string, v: unknown) => void }> = ({
  p,
  set,
}) => (
  <FieldGroup>
    <div>
      <Label>Canal</Label>
      <Select
        value={(p.channel as NotificationChannel) ?? "slack"}
        onChange={(e) => set("channel", e.target.value)}
      >
        {(
          ["slack", "teams", "email", "discord", "webhook", "pagerduty"] as NotificationChannel[]
        ).map((c) => (
    ...
    <div>
      ...
      <Select
        value={(p.notifyOn as NotificationTrigger) ?? "always"}
        onChange={(e) => set("notifyOn", e.target.value)}
      >
    ...
    <div>
      ...
      <Input
        value={(p.recipient as string) ?? ""}
        onChange={(e) => set("recipient", e.target.value)}
        placeholder="#channel ou https://..."
      />
    </div>
    <div>
      <Label>Message</Label>
      <Textarea
        ...
        placeholder="Pipeline {{status}} sur {{branch}}"
      />
    </div>
  </FieldGroup>
);
  ...
  return (
    <FieldGroup>
      <div>
        ...
        <Select
          value={(p.logicalOperator as string) ?? "AND"}
          onChange={(e) => set("logicalOperator", e.target.value)}
        >
      ...
      {conditions.map((cond, idx) => (
        <div
          key={idx}
          className="rounded-lg border border-gray-200 dark:border-gray-700 p-2.5 flex flex-col gap-2"
        >
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => updateRow(idx, { leftOperand: e.target.value })}
              placeholder="${{ env.BRANCH }}"
            />
          </div>
          <div>
          ...
          {!noRight(cond.operator) && (
            <div>
              ...
              <Input
                ...
                onChange={(e) => updateRow(idx, { rightOperand: e.target.value })}
              />
            </div>
          )}
        </div>
...
const InvokableForm: React.FC<{ p: PP; set: (k: string, v: unknown) => void }> = ({ p, set }) => (
  <FieldGroup>
    <div>
      <Label>Job cible (meme pipeline)</Label>
      <Input
        value={(p.targetJobId as string) ?? ""}
        onChange={(e) => set("targetJobId", e.target.value)}
        placeholder="job-123"
      />
    </div>
    <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
      L'id doit correspondre a un job existant. Les cycles d'invocation sont refuses par le backend.
    </p>
  </FieldGroup>
);
...
const ParamsForm: React.FC<{ type: NodeType; p: PP; set: (k: string, v: unknown) => void }> = ({
  type,
  p,
  set,
}) => {
  switch (type) {
    case "trigger":
      return <TriggerForm p={p} set={set} />;
    case "shell_command":
      return <ShellForm p={p} set={set} />;
    case "docker":
      return <DockerForm p={p} set={set} />;
    case "git":
      return <GitForm p={p} set={set} />;
    case "test":
      return <TestForm p={p} set={set} />;
    case "build":
      return <BuildForm p={p} set={set} />;
    case "deploy":
      return <DeployForm p={p} set={set} />;
    case "notification":
      return <NotificationForm p={p} set={set} />;
    case "condition":
      return <ConditionForm p={p} set={set} />;
    case "invokable":
      return <InvokableForm p={p} set={set} />;
  }
};
    ...
    <div className="absolute top-0 right-0 h-full w-90 z-20 flex flex-col bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
        ...
        <div>
          ...
          <ParamsForm type={nodeType} p={localData.params.baseParameters} set={setParam} />
        </div>
      </div>
