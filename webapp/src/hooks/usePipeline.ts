...
function rfEdgeToGraphEdge(e: RFEdge): GraphEdge {
  return {
    id: e.id,
    source: e.source,
    target: e.target,
...
export const usePipeline = (projectId?: string, pipelineId?: string) => {
  ...
  const enterJob = useCallback(
    (jobId: string) => {
      ...
      const stepRFEdges = (fakeJob.stepEdges ?? []).map(stepEdgeToRF);

      setNodes(stepNodes);
      setEdges(stepRFEdges);
